...
const char* mqttServer = "telemitrie.zonnebootteam.nl";
...
int indicator_leds[] = {2, 3, 5};
enum indicator_color {RED, YELLOW, GREEN};
...
void lees_accu()
{
    ...
    else if (canMsg.data[3] == 14)
    {
      ...
      if (send_per_min(&accu_ms.bms_state))
      {
        ...
        client.publish(topic.c_str(), String(concat_uint32(canMsg.data[3], canMsg.data[4],
                                             canMsg.data[5], canMsg.data[6])).c_str());
      }
    }
  ...
  {
    Serial.println("failed to resolve can_id:");
    print_msg_raw();
    return;
  }
}
...
void lees_PitchAndRoll() {
  ...
  char text[63];
  snprintf(text, 63, "%s,%s", roll_text, pitch_text);
  client.publish("P/R", text);
}
...
{
  if (state < indicator_leds_len)
  {
    for (u8 i = 0; i < indicator_leds_len; ++i)
    {
      digitalWrite(indicator_leds[i], i == state);
    }
  }
}
